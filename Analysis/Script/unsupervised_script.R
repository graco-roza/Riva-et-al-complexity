...
(figure_a<- terms_quartile %>% 
    filter(group != "Overall") %>% 
mutate(term_num = rev(as.numeric(term)-1)) %>% 
 ggplot(aes(ymin=term_num,xmin=as.numeric(Quartile) + 1)) +
 facet_wrap(~group, scales="free_x") +
 geom_rect(data = ~ .x %>%  filter(Quartile != "Total"), aes(ymax=term_num + .9,xmax=as.numeric(Quartile) + 2, fill=group, alpha=n/100), colour="gray70")+
 geom_rect(data = ~ .x %>%  filter(Quartile == "Total"),aes(ymax=term_num + .9,xmax=as.numeric(Quartile) + 2), fill=NA, colour=NA)+
 geom_text(data = ~ .x %>%  filter(Quartile != "Total"), aes(y = term_num +.45, x = as.numeric(Quartile) + 1.5, label=n), inherit.aes=TRUE, colour="black")+
 geom_text(data = ~ .x %>%  filter(Quartile == "Total"), aes(y = term_num +.45, x = as.numeric(Quartile) + 1.5, label=n, colour=group, fontface= bold), inherit.aes=TRUE)+
...
theme_void()+
 theme(legend.position="none",
       ...
       ,strip.text.x = element_text(size=12, face="bold", vjust=1.1)
...
figure_b<-terms_mean %>% 
  ...
  #geom_jitter(alpha=.15, position = position_jitterdodge(jitter.width=0.4, jitter.height =0)) +
  geom_errorbarh(aes(xmin=lower.ci.beta, xmax=upper.ci.beta, fill= group, colour=group), position = position_dodge(0.9))+
  geom_point(aes(x=mean.beta, fill=group, colour=group, group=group), shape=21, size=2, position = position_dodge(0.9))+
...
figure_a + figure_b + plot_layout(widths=c(.7,.3))
